...
export default function Hero() {
  ...
  useEffect(() => {
    const randomStudents = Math.floor(Math.random() * (650 - 250 + 1)) + 250;
  ...
  useGSAP(() => {
    ...
    tl.fromTo(".hero-eyebrow", { clipPath: "inset(0 100% 0 0)", opacity: 1 }, { clipPath: "inset(0 0% 0 0)", duration: 0.9 })
    ...
    gsap.to(".hero-scroll-hint", { y: 8, duration: 1.2, repeat: -1, yoyo: true, ease: "sine.inOut" });
  ...
  return (
    <section ref={ref} className="relative w-full min-h-screen overflow-hidden">
      ...
      <div className="hero-overlay absolute inset-0 z-10 pointer-events-none"
        style={{ background: "linear-gradient(135deg, rgba(15,61,46,0.82) 0%, rgba(15,61,46,0.55) 50%, rgba(15,61,46,0.75) 100%)" }} />
      ...
      <div className="absolute inset-0 z-10 pointer-events-none"
        style={{ background: "radial-gradient(ellipse 80% 60% at 30% 50%, rgba(46,139,87,0.15) 0%, transparent 70%)" }} />
      ...
      <div className="relative z-20 w-full min-h-screen flex flex-col justify-center px-8 md:px-16 lg:px-24 pt-28 pb-20">
        <div className="max-w-3xl">
          ...
          <div className="hero-eyebrow inline-flex items-center gap-2.5 mb-8 px-4 py-2 rounded-full border"
          ...
          <div className="overflow-hidden mb-1">
            <div className="hero-line text-5xl md:text-6xl lg:text-[80px] font-bold text-white leading-none tracking-tight">Beyond Grades</div>
          </div>
          <div className="overflow-hidden mb-1">
            <div className="hero-line text-5xl md:text-6xl lg:text-[80px] font-bold leading-none tracking-tight text-gradient-gold">The UYGW</div>
          </div>
          <div className="overflow-hidden mb-8">
            <div className="hero-line text-5xl md:text-6xl lg:text-[80px] font-bold text-white leading-none tracking-tight">Learning Journey.</div>
          </div>

          <p className="hero-sub text-lg md:text-xl leading-relaxed max-w-xl mb-10"
          ...
          <div className="flex gap-4 flex-wrap mb-16">
            <button
              ...
              style={{ background: "linear-gradient(135deg, #2E8B57, #165C44)", color: "#FFFFFF", boxShadow: "0 8px 32px rgba(46,139,87,0.5)" }}
            >
            ...
            <button
              ...
              style={{ color: "#FFFFFF", background: "rgba(255,255,255,0.1)", borderColor: "rgba(255,255,255,0.25)", backdropFilter: "blur(12px)" }}
            >
              ▶ Watch Preview
            </button>
          </div>
          ...
          <div className="flex gap-10 flex-wrap">
            {[
              ...
              <div key={s.label} className="hero-stat">
                <div className="stat-num text-3xl md:text-4xl font-bold text-white"
                  data-end={s.end} data-suffix={s.suffix} data-dec={s.dec}
                  style={{ textShadow: "0 0 30px rgba(212,175,55,0.5)" }}>
                  {s.end}{s.suffix}
                </div>
                <div className="text-xs mt-0.5 tracking-wide uppercase" style={{ color: "#C9D6CC" }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>

        <div className="absolute bottom-24 right-8 md:right-16 flex items-center gap-3 px-4 py-3 rounded-2xl border"
          style={{ background: "rgba(15,61,46,0.65)", borderColor: "rgba(255,255,255,0.15)", backdropFilter: "blur(16px)" }}>
          ...
          <div>
            ...
            <div className="text-[10px]" style={{ color: "#C9D6CC" }}>{studentsWatching.toLocaleString()} students watching</div>
          </div>
        </div>

        <div className="hero-scroll-hint absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 opacity-60">
          <span className="text-[10px] tracking-widest uppercase" style={{ color: "#C9D6CC" }}>Scroll</span>
